...
help.start()
# Go to > An introduction to R > Appendix A - A sample session

# Follow instructions, also look at help pages (R documentation) for different 
# commands/functions and do own exploratory analysis to understand what is happening and why
...
summary(x)
mean(x)   # mean close to 0
sd(x)     # standard deviation close to 1

y <- rnorm(x)  # number of observations is equal to number of observations in x, that is n = 50
y
...
# The next section will look at data from the classical experiment of Michelson to measure 
# the speed of light. This dataset is available in the morley object, but we will read it 
# to illustrate the read.table function.
